...
/**
 * Berechnet die Differenz zwischen zwei timespec-Strukturen in Millisekunden.
 * @param start Startzeitpunkt.
 * @param end Endzeitpunkt.
 * @return Differenz in Millisekunden. Falls end < start, wird 0 zurückgegeben.
 */
static double timespec_diff_ms(const struct timespec *start, const struct timespec *end) {
    ...
    // Sicherstellen, dass die Latenz nicht negativ ist
    if (latency_ms < 0.0) {
        return 0.0;
    }
...
static void match_n6_packet(const unsigned char *payload, size_t len, const struct timespec *ts_n6) {
    ...
    while (curr) {
        if (curr->payload_len == len && memcmp(curr->payload, payload, len) == 0) {
            // Berechne Latenz in Millisekunden
