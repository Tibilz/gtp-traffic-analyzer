 ...
 * Ein Programm zur Überwachung und Analyse von UDP-Paketverkehr mithilfe von pcap.
 *
 * Dieses Programm analysiert UDP-Paketflüsse auf den N3- und N6-Schnittstellen in einem 5G-Netz.
...
// Konstanten
#define HASHMAP_SIZE 65536 // Erhöhte Größe zur Minimierung von Kollisionen
...
static int parse_gtp_header(const unsigned char *data, size_t len) {
    ...
    if (e_flag) {
        while (1) {
            ...
            if (next_ext_type == 0) break; // Ende der Extensions
            if (offset >= len) return -1;
            unsigned char ext_len = data[offset++];
            if (offset + ext_len > len) return -1;
            offset += ext_len;
        }
    }
    return offset;
...
/**
 * Extrahiert die Anwendungspayload aus einem N3-Paket.
...
static unsigned char* extract_payload_n3(const u_char *packet, const struct pcap_pkthdr *hdr, size_t *payload_len) {
    ...
    // Ethernet
    if (remaining < 14) {
        printf("N3: Paket zu kurz für Ethernet-Header. Länge: %zu\n", remaining);
        return NULL;
    }
    p += 14;
    remaining -= 14;
    ...
    // Äußerer IP-Header
    int outer_ip_len = parse_ip_header(p, remaining);
    if (outer_ip_len < 0) {
        printf("N3: Ungültiger äußerer IP-Header. Länge: %zu\n", remaining);
        return NULL;
    }
    p += outer_ip_len;
    remaining -= outer_ip_len;

    // Äußerer UDP-Header
    int outer_udp_len = parse_udp_header(p, remaining);
    if (outer_udp_len < 0) {
        printf("N3: Ungültiger äußerer UDP-Header. Länge: %zu\n", remaining);
        return NULL;
    }
    p += outer_udp_len;
    remaining -= outer_udp_len;

    // GTP-U-Header
    int gtp_len = parse_gtp_header(p, remaining);
    if (gtp_len < 0) {
        printf("N3: Ungültiger GTP-U-Header. Länge: %zu\n", remaining);
        return NULL;
    }
    p += gtp_len;
    remaining -= gtp_len;
    ...
    // Innerer IP-Header
    if (remaining < 20) {
        printf("N3: Paket zu kurz für inneren IP-Header. Länge: %zu\n", remaining);
        return NULL;
    }
    int inner_ip_len = parse_ip_header(p, remaining);
    if (inner_ip_len < 0) {
        printf("N3: Ungültiger innerer IP-Header. Länge: %zu\n", remaining);
        return NULL;
    }
    p += inner_ip_len;
    ...
    // Innerer UDP-Header
    if (remaining < 8) {
        printf("N3: Paket zu kurz für inneren UDP-Header. Länge: %zu\n", remaining);
        return NULL;
    }
    int inner_udp_len = parse_udp_header(p, remaining);
    if (inner_udp_len < 0) {
        printf("N3: Ungültiger innerer UDP-Header. Länge: %zu\n", remaining);
        return NULL;
    }
    p += inner_udp_len;
    ...
    if (!pl) {
        printf("N3: Speicherzuweisung fehlgeschlagen für Payload.\n");
        return NULL;
    }
    memcpy(pl, p, remaining);
    *payload_len = remaining;

    // Debugging: Erste 16 Bytes der Payload anzeigen
    printf("N3: Extrahierte Payload-Länge: %zu Bytes. Erste 16 Bytes: ", remaining);
    for (size_t i = 0; i < (remaining < 16 ? remaining : 16); i++) {
        printf("%02x ", pl[i]);
    }
    printf("\n");
...
/**
 * Extrahiert die Anwendungspayload aus einem N6-Paket.
 * @param packet Zeiger auf die Paketdaten.
 * @param hdr Paketmetadaten (pcap Header).
 * @param payload_len Zeiger zum Speichern der extrahierten Payload-Länge.
 * @return Zeiger auf die extrahierte Payload bei Erfolg, NULL bei Fehler.
...
static unsigned char* extract_payload_n6(const u_char *packet, const struct pcap_pkthdr *hdr, size_t *payload_len) {
    ...
    if (ip_len < 0) {
        printf("N6: Ungültiger IP-Header. Länge: %zu\n", remaining);
        return NULL;
    }
    p += ip_len;
    remaining -= ip_len;
    ...
    if (udp_len < 0) {
        printf("N6: Ungültiger UDP-Header. Länge: %zu\n", remaining);
        return NULL;
    }
    p += udp_len;
    remaining -= udp_len;
    ...
    unsigned char *pl = malloc(remaining);
    if (!pl) {
        printf("N6: Speicherzuweisung fehlgeschlagen für Payload.\n");
        return NULL;
    }
    memcpy(pl, p, remaining);
    *payload_len = remaining;
    ...
    // Debugging: Erste 16 Bytes der Payload anzeigen
    printf("N6: Extrahierte Payload-Länge: %zu Bytes. Erste 16 Bytes: ", remaining);
    for (size_t i = 0; i < (remaining < 16 ? remaining : 16); i++) {
        printf("%02x ", pl[i]);
    }
    printf("\n");
...
static void insert_n3_packet(const unsigned char *payload, size_t len, const struct timeval *ts) {
    ...
    if (!entry) {
        printf("N3: Speicherzuweisung für packet_entry_t fehlgeschlagen.\n");
        return;
    }
    entry->payload = malloc(len);
    if (!entry->payload) {
        printf("N3: Speicherzuweisung für Payload fehlgeschlagen.\n");
        free(entry);
        return;
    }
...
static void match_n6_packet(const unsigned char *payload, size_t len, const struct timeval *ts_n6) {
    ...
    while (curr) {
        if (curr->payload_len == len && memcmp(curr->payload, payload, len) == 0) {
            ...
            // Debugging: Gematchtes Paket
            printf("Match gefunden! Latenz: %.2f ms\n", latency_ms);
            return;
        }
        prev = curr;
        curr = curr->next;
    ...
    // Debugging: Kein Match gefunden
    printf("Kein Match für N6-Paket gefunden. Payload-Länge: %zu Bytes\n", len);
}
        ...
        for (int i = 0; i < HASHMAP_SIZE; i++) {
            ...
            while (curr) {
                ...
                if (elapsed > TIMEOUT_SECONDS) {
                    ...
                    // Debugging: Paket als verloren markiert
                    // printf("Paket verloren. Elapsed: %.2f s\n", elapsed);
                } else {
                    prev = curr;
                    curr = curr->next;
                }
