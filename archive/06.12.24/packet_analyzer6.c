...
// Funktion zur Berechnung der tatsächlichen GTPv1-Header-Länge basierend auf den Flags und Optionen
size_t calculate_gtp_header_length(struct gtpv1_header *gtp, const u_char *packet, size_t caplen, size_t ip_header_len) {
    size_t header_len = sizeof(struct gtpv1_header); // 8 Bytes
    size_t extension_len = 0;
    const size_t MAX_EXTENSION_LEN = 256; // Maximale Erweiterungsheader-Länge zur Vermeidung von Überläufen
    const size_t MAX_EXTENSION_OPTIONS = 10; // Maximale Anzahl der zu parsenden Erweiterungsoptionen

    // Bit 5 (0x20) ist das Extension Header Flag (E-Flag)
    if (gtp->flags & 0x20) { // E-Flag gesetzt
        const u_char *ext_ptr = (const u_char *)gtp + sizeof(struct gtpv1_header);
        size_t max_extension_len = caplen - sizeof(struct ether_header) - ip_header_len - sizeof(struct udphdr) - sizeof(struct gtpv1_header);

        LOG(LOG_DEBUG, "Parsing GTPv1 extension headers. Max extension length: %zu bytes", max_extension_len);

        size_t parsed_options = 0;

        // Beispielhafte Annahme: Erweiterungsoptionen folgen dem Typ-Length-Value (TLV)-Format
        while (ext_ptr + 2 <= packet + caplen && extension_len + 2 <= max_extension_len && parsed_options < MAX_EXTENSION_OPTIONS) {
            uint8_t option_type = *ext_ptr++;
            uint8_t option_length = *ext_ptr++;

            LOG(LOG_DEBUG, "Parsed GTPv1 extension option: type=%u, length=%u", option_type, option_length);

            if (option_length == 0) { // Vermeidung einer Endlosschleife
                LOG(LOG_WARNING, "GTPv1 extension option length is 0, stopping parsing");
                break;
            }

            if (extension_len + 2 + option_length > max_extension_len) {
                LOG(LOG_WARNING, "Incomplete GTPv1 extension option value. Option type: %u, length: %u", option_type, option_length);
                break;
            }

            // Hinzufügen der Option zur Header-Länge
            header_len += 2 + option_length;
            extension_len += 2 + option_length;
            parsed_options++;

            LOG(LOG_DEBUG, "Total extension_len after parsing: %zu bytes", extension_len);

            if (option_type == 0) { // Option Type 0 könnte das Ende anzeigen
                LOG(LOG_DEBUG, "GTPv1 extension option type 0 encountered, stopping parsing");
                break;
            }

            // Sicherheitsprüfung: Verhindern, dass header_len zu groß wird
            if (header_len > MAX_EXTENSION_LEN) {
                LOG(LOG_WARNING, "GTPv1 header length exceeded maximum allowed (%zu bytes). Stopping parsing.", MAX_EXTENSION_LEN);
                break;
            }
        }

        if (parsed_options >= MAX_EXTENSION_OPTIONS) {
            LOG(LOG_WARNING, "Reached maximum number of extension options (%zu). Stopping parsing.", (size_t)MAX_EXTENSION_OPTIONS);
        }

        LOG(LOG_DEBUG, "Final GTPv1 extension_len: %zu bytes", extension_len);
    }

    // Bit 6 (0x40) ist das Sequence Number Flag (S-Flag)
    if (gtp->flags & 0x40) { // S-Flag gesetzt
        header_len += 2; // Sequenznummer ist 2 Bytes
        LOG(LOG_DEBUG, "Sequence Number Flag gesetzt. Erweiterte Header-Länge um 2 Bytes.");
    }

    // Bit 7 (0x80) ist das N-PDU Number Flag (PN-Flag)
    if (gtp->flags & 0x80) { // PN-Flag gesetzt
        header_len += 1; // N-PDU-Nummer ist 1 Byte
        LOG(LOG_DEBUG, "N-PDU Number Flag gesetzt. Erweiterte Header-Länge um 1 Byte.");
    }

    LOG(LOG_DEBUG, "Calculated GTPv1 Header Length: %zu bytes", header_len);
    return header_len;
}
...
void process_n3_packet(u_char *user, const struct pcap_pkthdr *header, const u_char *packet) {
    ...
    if (header->caplen < (sizeof(struct ether_header) + sizeof(struct iphdr) + sizeof(struct udphdr) + sizeof(struct gtpv1_header))) {
    ...
    // Berechne die tatsächliche GTPv1-Header-Länge basierend auf den Flags und Optionen
    size_t gtp_header_len = calculate_gtp_header_length((struct gtpv1_header *)gtp, packet, header->caplen, ip_header_len);
    ...
    size_t gtp_length_field = ntohs(gtp->length);
    size_t payload_len = gtp_length_field;

    // Berechne die verbleibende Kapazität für den Payload
    size_t remaining_caplen = header->caplen - headers_len;

    LOG(LOG_DEBUG, "GTPv1 length field: %zu, remaining_caplen: %zu", payload_len, remaining_caplen);

    // Überprüfung der Konsistenz mit ip->tot_len
    size_t ip_tot_len = ntohs(ip->tot_len);
    size_t expected_payload_len = ip_tot_len - ip_header_len - sizeof(struct udphdr) - gtp_header_len;

    LOG(LOG_DEBUG, "IP Total Length: %zu, Expected Payload Length: %zu", ip_tot_len, expected_payload_len);

    if (payload_len != expected_payload_len) {
        LOG(LOG_WARNING, "GTPv1 length field (%zu) does not match expected payload length (%zu)", payload_len, expected_payload_len);
        // Entscheiden, ob payload_len angepasst werden soll oder das Paket übersprungen werden soll
        // Hier als Beispiel: payload_len auf expected_payload_len anpassen, wenn payload_len > expected_payload_len
        if (payload_len > expected_payload_len) {
            LOG(LOG_WARNING, "Adjusting payload_len from %zu to expected %zu", payload_len, expected_payload_len);
