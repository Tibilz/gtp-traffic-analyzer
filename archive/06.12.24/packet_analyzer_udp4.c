...
static struct timeval start_time; // Global declaration of start_time
...
/**
 * Extracts the application payload from an N3 packet by skipping the first 94 bytes.
...
static unsigned char* extract_payload_n3(const u_char *packet, const struct pcap_pkthdr *hdr, size_t *payload_len) {
    ...
    // 1. Skip first 94 bytes for N3 packets - remove all headers before the payload
    if (remaining < 94) {
        return NULL;
    }
    p += 94; 
...
/**
 * Extracts the application payload from an N6 packet by skipping the first 40 bytes.
 * @param packet Pointer to the packet data.
 * @param hdr Packet metadata (pcap Header).
 * @param payload_len Pointer to store the extracted payload length.
 * @return Pointer to the extracted payload on success, NULL on error.
...
static unsigned char* extract_payload_n6(const u_char *packet, const struct pcap_pkthdr *hdr, size_t *payload_len) {
    ...
    // 1. Skip first 40 bytes for N6 packets
    if (remaining < 40) {
        return NULL;
    }
    p += 40;
...
static void *cleanup_thread_func(void *arg) {
    (void)arg;
    while (running) {
        usleep(CLEANUP_INTERVAL_MS * 1000); // usleep is now declared
...
int main(int argc, char **argv) {
    ...
    if (pcap_compile(handle_n3, &fp_n3, "src host 10.2.2.154  udp port 2152", 0, PCAP_NETMASK_UNKNOWN) == -1) {
