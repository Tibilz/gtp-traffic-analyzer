 ...
 * Ein Programm zur Überwachung und Analyse von Paketverkehr mithilfe von Raw Sockets.
 *
 * Dieses Programm analysiert GTP-U-Paketflüsse auf den N3- und N6-Schnittstellen in einem 5G-Netz.
 ...
 *
 * Funktionen:
 * - Paketextraktion und Parsing für N3- und N6-Pakete.
...
int main(int argc, char **argv) {
    ...
    if (pcap_compile(handle_n3, &fp_n3, "src host 10.2.2.154 and udp port 2152", 0, PCAP_NETMASK_UNKNOWN) == -1) {
        fprintf(stderr, "Fehler beim pcap_compile N3\n");
        return 1;
    }
    if (pcap_setfilter(handle_n3, &fp_n3) == -1) {
        fprintf(stderr, "Fehler beim pcap_setfilter N3\n");
        return 1;
    }
    pcap_freecode(&fp_n3);
    ...
    if (pcap_compile(handle_n6, &fp_n6, "dst host 10.2.2.157 and udp port 5201", 0, PCAP_NETMASK_UNKNOWN) == -1) {
        fprintf(stderr, "Fehler beim pcap_compile N6\n");
        return 1;
    }
    if (pcap_setfilter(handle_n6, &fp_n6) == -1) {
        fprintf(stderr, "Fehler beim pcap_setfilter N6\n");
        return 1;
    }
    pcap_freecode(&fp_n6);
